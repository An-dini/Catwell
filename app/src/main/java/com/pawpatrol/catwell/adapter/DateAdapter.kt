...
class DateAdapter(private val dateList: List<DateModel>) : RecyclerView.Adapter<DateAdapter.DateViewHolder>() {
    ...
    override fun onBindViewHolder(holder: DateViewHolder, position: Int) {
        ...
        if (position == selectedItemPosition) {
            ...
            holder.dayTextView.setTextColor(ContextCompat.getColor(holder.itemView.context, R.color.orange))
            holder.dayOfWeekTextView.setTextColor(ContextCompat.getColor(holder.itemView.context, R.color.orange))
        } else {
